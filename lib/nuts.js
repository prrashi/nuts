...
function getBasePath (req) {
  ...
  const {"x-base-url": baseUrl = ""} = req.headers;
...
function getFullUrl(req) {
    ...
    return new URL('https://' + req.get('host') + getBasePath(req) + req.originalUrl);
}
...
function Nuts(opts) {
    ...
    _.bindAll(this, _.keys(Nuts.prototype));
...
Nuts.prototype.onDownload = function(req, res, next) {
    ...
    .then(function(version) {
        ...
        if (!asset) throw new Error("No download available for platform "+platform+" for version "+version.tag+" ("+(channel || "beta")+")" + (arch ? " " + arch : ""));
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        const {protocol, hostname} = fullUrl,
              basePath = getBasePath(req);

        var releaseNotes = notes.merge(notesSlice, { includeTag: false });
        console.error(latest.tag);
        res.status(200).send({
            "url": urljoin(`${protocol}//${hostname}`, basePath, '/download/version/'+latest.tag+'/'+platform+'?filetype='+filetype + (arch ? "&arch=" + arch : "")),
            "name": latest.tag,
            "notes": releaseNotes,
       ...
       .then(function(content) {
            ...
            releases = _.chain(releases)
                ...
                .map(function(entry) {
                    ...
                    entry.filename = urljoin(`${protocol}//${hostname}`, basePath, '/download/'+entry.semver+'/'+entry.filename);
