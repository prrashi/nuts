...
function getFullUrl(req) {
    ...
    let {"x-base-url": baseUrl = ""} = req.headers;

    return req.protocol + '://' + req.get('host') + baseUrl + req.originalUrl;
}
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        res.status(200).send({
            "url": urljoin(fullUrl, gitFilePath, '/download/version/'+latest.tag+'/'+platform+'?filetype='+filetype + (arch ? "&arch=" + arch : "")),
            "name": latest.tag,
            "notes": releaseNotes,
