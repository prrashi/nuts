...
var ARCH_ARM_64 = "arm64";
var ARCH_X_64 = "x64";

function extractArch (platform, fileName) {

    // TODO: need to do for other platforms also
    if (_.includes(platform, platforms.OSX)) {

        if(_.includes(fileName, ARCH_ARM_64)) {
    ...
    var releasePlatforms = _.chain(release.assets)
        .map(function(asset) {
            ...
            return {
                ...
                raw: asset,
                arch: extractArch(platform, asset.name)
            };
        })
        .compact()
        .value();
    ...
    .then(function(versions) {
        return _.chain(versions)
            .filter(function(version) {
                ...
                if (opts.platform && !platforms.satisfies(opts.platform, _.map(version.platforms, 'type'))) return false;
